...
// piaKeepalivePacketType is the undocumented PRUDP packet type 8 that SSBU's Pia
...
// keepalive went un-ACKed and SSBU treated the connection as dead → soft-lock when
// entering "En ligne". ACKing it unblocks the online menu. Harmless for MK8 (never
// sends type 8).
const piaKeepalivePacketType uint8 = 8

func setupSSBUType8Keepalive(endpoint *nex.Endpoint) {
	endpoint.RegisterCustomPacketHandler(piaKeepalivePacketType, func(c *nex.Connection, p *nex.Packet) {
		...
		fmt.Printf("[SSBU Type8] Pia keepalive seqID=%d -> ACK\n", p.PacketID)
	})
}
